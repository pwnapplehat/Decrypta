...
### Fixed
- **A failed decrypt no longer shows "Decryption complete" with an older IPA from your Library.**
  If the device is missing `appinst` (or anything else stops decryption after download), ipadecrypt
  can still exit 0 without writing a new file. Decrypta was then picking the newest IPA already in
  the output folder (e.g. claiming `…_439.0.0.ipa` after a failed `440.0.0` run) and showing success.
  It now only accepts an IPA this run actually wrote, treats "exit 0 but no new IPA" as failure, and
  the dialog/log surface the real error (including an appinst / Sign-in bootstrap hint).
